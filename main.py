...
def register():
    ...
    if form.validate_on_submit():
        if User.query.filter_by(email=form.email.data).first():
            flash("You have already registered with this email address. Try to log in.")
...
def show_post(post_id):
    ...
    # comments = Comment.query.filter_by(blogpost_id=requested_post.id)
    form = CommentForm()
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            flash("You have to log in to be able to comment. Please, log in!")
